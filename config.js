...
module.exports = {
  ...
  AI_API: {
    ...
    models: {
      // 文本模型
      textModels: process.env.TEXT_MODELS ? 
        JSON.parse(process.env.TEXT_MODELS) : 
        [
          { id: 'deepseek/deepseek-r1:free', name: 'DeepSeek R1', description: '预设的推理模型，适合专业讨论和问题解答' },
          { id: 'deepseek/deepseek-chat-v3-0324:free', name: 'DeepSeek Chat', description: '通用对话模型，适合日常对话和创意内容' }
        ],
      // 多模态模型（支持图像理解）
      multimodalModels: process.env.MULTIMODAL_MODELS ?
        JSON.parse(process.env.MULTIMODAL_MODELS) :
        [
